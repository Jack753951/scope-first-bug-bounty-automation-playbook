...
```text
OBSIDIAN_VAULT_PATH=<user-home>/Documents/ObsidianProjects
...
```powershell
New-Item -ItemType Directory -Force "<user-home>\Documents\ObsidianProjects"
```
...
```text
<user-home>/AppData/Local/hermes/.env
```
...
```bash
powershell -NoProfile -ExecutionPolicy Bypass -Command '$vault="<user-home>/Documents/ObsidianProjects"; New-Item -ItemType Directory -Force -Path $vault | Out-Null; Write-Output "VAULT=$vault"'
```
