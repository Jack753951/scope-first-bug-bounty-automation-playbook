...
- `<attacker-vm>`: attacker/tester plane for target-touching tools, browser/runtime checks, curl/nmap/ffuf/nikto/sqlmap-class work, artifact collection.
- `<victim-vm>`: Docker target host for vulnerable apps such as Juice Shop and WebGoat/WebWolf.
...
- `<attacker-vm>`: host-only IP commonly `<lab-ip>`.
- `<victim-vm>`: host-only IP commonly `<lab-ip>`.
