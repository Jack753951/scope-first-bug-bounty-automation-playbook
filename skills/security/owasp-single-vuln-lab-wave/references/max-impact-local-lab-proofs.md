...
- target: Docker-published DVWA on `<victim-vm>`;
- tester: `<attacker-vm>`;
...
- outbound callback was not verified in the first run because high-port listeners on `<attacker-vm>` and Windows host were unreachable from the target path;
...
- future reruns should use the current attacker route `<attacker-vm>`; older artifact labels that say `<attacker-vm>` are historical metadata unless project handoff supersedes this;
