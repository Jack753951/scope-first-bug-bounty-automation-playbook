...
- Put full third-party repos, payload archives, and raw runtime outputs under git-ignored local paths such as `setting/local/...` and `<artifact-output-dir>/...`.
...
<artifact-output-dir>/<run>/                             # git-ignored runtime outputs
...
```bash
! -path '*/setting/local/*' ! -path '*/<artifact-output-dir>/*'
```
