...
# Phase 5A Authorized Live-Target Dry-Run Template

Status: active template / no target authorized by this file
Date: 2026-05-23
Purpose: move Cybersec Lab from local proof platform to controlled authorized-assessment readiness without automatic public/live target touching.

## Boundary

This template does not authorize testing. It only defines what must be collected before Hermes, Kali, scripts, or a human operator touches a live/real target.
...
If any line is unknown and materially affects safety, ask the operator before proceeding.
...
Stop immediately if behavior crosses the approved scope or requires an unapproved action.
...
Do not store tokens, passwords, API keys, private user data, or raw sensitive records. Use `[REDACTED]` when unavoidable in summaries.
...
```text
handoff/phase5a_<target_alias>_scope_interpretation_YYYYMMDD.md
handoff/phase5a_<target_alias>_target_map_YYYYMMDD.md
handoff/phase5a_<target_alias>_candidate_lanes_YYYYMMDD.md
handoff/phase5a_<target_alias>_evidence_packet_YYYYMMDD.md
handoff/phase5a_<target_alias>_report_decision_YYYYMMDD.md
```

Raw/redacted evidence should stay in an explicit artifact folder, not global memory.
