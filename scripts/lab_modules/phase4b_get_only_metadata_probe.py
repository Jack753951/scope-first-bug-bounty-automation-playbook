#!/usr/bin/env python3
"""Phase 4B GET-only lab fast-lane adapter.

The adapter is intentionally narrow: fixed/private lab targets, fixed GET-only
metadata paths, health gates, capped request counts, temporary raw bodies, and
candidate-only JSONL observations. It is a reusable replacement for the one-off
`setting/local/phase4b_owasp_lab_probe.sh` helper.
"""
...
SCHEMA_VERSION = "phase4b_get_only_metadata_probe/0.1-trial"
...
GET_PATHS = [
    ("root_baseline", "level1.root_metadata", "/", "baseline root"),
    ("robots_txt", "level1.robots_securitytxt_metadata", "/robots.txt", "robots metadata"),
    ("security_txt", "level1.robots_securitytxt_metadata", "/.well-known/security.txt", "security.txt metadata"),
    ("ftp_listing", "level1.directory_listing_metadata", "/ftp/", "directory listing metadata"),
    ("api_docs", "level1.api_docs_metadata", "/api-docs/", "api docs metadata"),
    ("product_search_api", "level2.benign_parameter_metadata", "/rest/products/search?q=phase4b_canary", "API benign parameter canary"),
    ("spa_search", "level2.benign_parameter_metadata", "/search?q=phase4b_canary", "SPA fallback canary"),
    ("redirect_canary", "level2.open_redirect_candidate", "/redirect?to=https://phase4b-canary.invalid/", "open redirect negative/positive canary; no follow"),
]
...
def build_plan(
    *,
    ...
    for step_id, module_id, path, note in GET_PATHS:
        steps.append({
            "id": step_id,
            "kind": "get_metadata",
            "method": "GET",
...
def render_bash(plan: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        "record_get(){",
        "  local id=$1 module_id=$2 url=$3 logical_path=$4 note=$5",
        ...
        "  python3 - \"$id\" \"$module_id\" \"$url\" \"$logical_path\" \"$note\" \"$code\" \"$hdr\" \"$body\" \"$SNIPPET_BYTES\" >> \"$OUT/observations.jsonl\" <<'PY'",
        "import hashlib, json, pathlib, re, sys, datetime",
        "step_id, module_id, url, logical_path, note, curl_code, hdr_path, body_path, snippet_bytes = sys.argv[1:10]",
        ...
        "obs = {'schema_version':'phase4b_get_only_observation/0.1-trial','ts_utc':datetime.datetime.now(datetime.UTC).replace(microsecond=0).isoformat().replace('+00:00','Z'),'status':'candidate','kind':'get_metadata','module_id':module_id,'step_id':step_id,'url':url,'path':logical_path,'note':note,'http_status':status,'curl_code':curl_code,'content_type':headers.get('content-type'),'location_header':headers.get('location'),'title':title,'body_sha256':hashlib.sha256(body).hexdigest(),'body_size':len(body),'short_redacted_snippet':snippet,'directory_listing_candidate': logical_path == '/ftp/' and status == 200,'manual_verification_required':True,'promotes_findings':False}",
    ...
    for step in plan["steps"]:
        if step["kind"] != "get_metadata":
            continue
        lines.append(
            "record_get "
            f"{_sq(step['id'])} {_sq(step['module_id'])} {_sq(step['url'])} {_sq(step['path'])} {_sq(step['note'])}"
        )
        if delay:
    ...
    lines.extend([
        ...
        "    out.write('\\nnotable_paths\\n')",
        "    for r in rows: out.write(f\"GET {r.get('path')} -> {r.get('http_status')} {r.get('content_type')} len={r.get('body_size')} title={r.get('title')!r} loc={r.get('location_header')!r}\\n\")",
        "PY",
