...
## Direction-Alignment Before Project Cleanup

When the user says the next message is the project's future direction, north star, or long-term strategy — especially if they ask to reconcile nonconforming policy, global memory, long-term goals, or Obsidian — pause implementation and treat the statement as an authority update rather than ordinary chat.

Apply this sequence:

1. Acknowledge that the next user message is project-direction input and do not start file moves, commits, schema fixes, or runtime work yet.
2. Classify the direction across layers:
   - repo policy / agent contracts: `.hermes.md`, `AGENTS.md`, `docs/policy/`, runbooks;
   - repo operational truth: `handoff/current_navigation.md`, `handoff/active_strategy_queue.md`, accepted changes, current lane state;
   - Hermes durable memory: compact cross-session signposts only;
   - Obsidian project namespace: long-term strategy, decision rationale, review synthesis, and superseded/rejected direction notes.
3. Identify conflicts explicitly as `keep`, `rewrite`, `rehome to Obsidian`, `remove/compact global memory`, `mark superseded`, or `needs user confirmation`.
4. Prefer a patch plan first. Do not immediately mutate policy, memory, or Obsidian unless the user explicitly says to update/apply.
5. If applying changes, update project-local authority layers before compacting global memory so future agents can still recover the rationale from repo/Obsidian.
6. Never store secrets, raw target details, cookies, tokens, loot, private scope/rules, or account-sensitive evidence in global memory or Obsidian while reconciling direction.

This pattern is especially important after repository cleanup discussions: do not let commit grouping, file relocation, schema migration, or multi-agent orchestration proceed until the user's future-shape assumptions are aligned.
...
See `references/high-risk-activation-gate-routing.md` for the pattern to use when a project asks to move from internal/readiness work into live scheduling, upload, publication, OAuth, destination, privacy, or other production-affecting activation.
...
See `references/risk-tiered-local-learning-canary-cadence.md` for the pattern to use when a media/automation project is over-gated: keep upload/publication/OAuth/scheduler gates strict, but make local-only learning fast and move positive Claude/visionreview candidates toward exact-artifact canaries for live data.
...
See `references/sibling-project-structure-sync-routing.md` for the pattern to use when the user asks to read a cleaned clone/sibling repo and optimize the main repo from it: borrow compact active-truth structure, preserve the main repo's driver model and lane state, and defer any sibling lane/KILL decisions to a separate reconciliation pass.
...
See `references/cybersec-platform-direction-sync-routing.md` for the pattern to use when the Cybersec Lab operator states or corrects the future direction/north star: wait until the operator finishes, compact global memory, update `.hermes.md`, current navigation, active queue, Obsidian bridge, and accepted changes, preserve bounded proof-only safety semantics for in-scope aggressive testing, and check active authority files for superseded first-bounty/passive-only language.
...
See `references/project-as-capability-library-routing.md` for the pattern to use when the user frames a project as a dedicated, growing Hermes/agent capability library rather than human-readable narrative: route operational details to project authority layers, favor reusable scripts/contracts/validation/handoff substrate, and keep only compact global signposts.
...
25. Treating a project-as-capability-library correction as a request for nicer documentation.
   - Fix: route the project framing into repo-local authority layers and capability substrate. Prefer runnable scripts, validation recipes, machine-readable queues/contracts, worker prompts, and compact artifact indexes over human-readable behavioral narratives. Save only compact global signposts and keep sensitive/project-specific details out of skills and durable memory. See `references/project-as-capability-library-routing.md`.

26. Copying a cleaned sibling repo wholesale into the main repo.
   - Fix: when using a cleaned clone/sibling as reference, borrow structure and authority-layer patterns only. Preserve the main repo's active driver model, safety gates, and machine-readable lane state unless the user explicitly approves a state reconciliation. See `references/sibling-project-structure-sync-routing.md`.
