...
- Attacker VM: `<lab-vm>`, observed host-only IP `<lab-ip>`
- Victim VM: `<lab-vm>`, observed host-only IP `<lab-ip>`
...
"$VBOX" controlvm <lab-vm> poweroff || true
"$VBOX" snapshot <lab-vm> restore pre-aggressive-current-running-recovery-20260521-093252
