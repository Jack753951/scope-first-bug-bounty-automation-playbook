...
# HackerOne logged-in scope intake via Kali/noVNC

Use this when the operator has already logged into HackerOne in the Kali/noVNC browser and explicitly authorizes work only inside the program's in-scope assets.

## Lesson

Do not ask the operator to manually copy a HackerOne asset table when Hermes can use the authorized remote UI session to read it. The operator's correction in this session was: if Kali/noVNC mastery is part of the platform goal and the H1 account is already logged in, Hermes should self-serve scope intake through the remote UI, not bounce the work back to the user.
...
2. Use Kali/noVNC, not a separate browser session, when the user's logged-in HackerOne session is already there.
3. Navigate to the program scope/policy page and prefer official export/download when available:
   - typical page: `https://hackerone.com/<program>/policy_scopes`;
