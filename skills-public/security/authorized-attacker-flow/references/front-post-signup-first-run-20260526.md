...
# Front post-signup first run — authorized attacker-flow example
...
- Program: Front / HackerOne `<program-redacted>`.
...
4. Stop at any external account/channel connection gate.
   - Example: Gmail, Office 365, Slack, WhatsApp, Twilio, Front Chat, webhooks, SMS/chat channels, or channel settings.
5. Convert visible surfaces into candidates rather than executing them immediately.

## Candidate set produced

- `front-channel-connect-oauth-boundary` — high impact, but requires owned mailbox/channel, OAuth/token redaction plan, and explicit operator control.
- `front-invite-role-boundary` — needs second owned account before invite/role tests.
- `front-shared-inbox-object-permission` — best next bounded lane, but start with passive UI/object inventory; object creation needs a named owned-object plan.
- `front-workflow-rule-abuse` — preserve as blocked until rule builder can be mapped without saving/enabling rules.
- `front-api-ui-permission-mismatch` — docs-first only until API token handling/redaction plan is approved.
