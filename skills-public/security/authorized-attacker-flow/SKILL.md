...
### Env-backed signup secret handling for this user

When an authorized live-bounty signup flow needs email/password/phone fields, this user's cybersec profile may provide reusable values through profile `.env` variables (for example fixed signup passwords and phone numbers). Treat these like API keys: they can be used to complete owned-account setup, but must not be copied into memory, repo artifacts, screenshots, handoff notes, or report drafts. Hermes may prefill email/password/phone from the configured rules/env and then alert the operator when SMS/email OTP, CAPTCHA, phone-code confirmation, KYC/payment, or final submit is needed. OTPs, verification links, cookies, bearer tokens, and API keys remain human/secret-gated and must not be stored.
...
Reference example: `references/front-post-signup-first-run-20260526.md` shows a Front post-signup run where onboarding choices were kept low-risk, channel connection became the hard stop-before, and the output was a five-candidate attacker-flow packet.
...
Env-backed signup reference: `references/env-backed-live-bounty-signup.md` records the user's delegated pattern for authorized live-bounty account setup: prefill deterministic H1 email plus env-backed password/phone when appropriate, then stop at OTP/CAPTCHA/KYC/payment/final-submit while keeping secrets out of durable artifacts.

Live-bounty secret/claim gate reference: `references/live-bounty-secret-prefill-and-private-claim-gates.md` records the user's H1 private-opportunity claim rule, H1 email prefill pattern, and `.env` handling for phone/password secrets. Use it before signup/account-control gates in live bounty work.
...
Practical review reference: `references/front-multi-agent-practical-review-20260526.md` shows the workflow doing its job on a tempting SaaS permission candidate: Claude preserved attacker hypotheses, Codex blocked premature proof execution, Hermes normalized a partial Claude artifact, and the lane stayed passive-only.

Fintech signup/API precondition reference: `references/fintech-signup-api-precondition-review.md` records the Syfe-like pattern for H1 fintech targets: capture scope/policy, map signup fields without submitting, inspect loaded public frontend config for API/auth/OTP/CAPTCHA indicators, do at most tiny fixed-path API root checks, build owned-object/API candidates, then stop at auth/OTP/CAPTCHA/phone/payment/KYC.
...
Account B passive surface reference: `references/front-account-b-passive-surface-map-20260526.md` records the pattern for post-operator-auth SaaS onboarding/dashboard mapping: keep lane state schema-valid, classify default owned objects as `surface_only`, preserve setup-guide gates, and run redaction/lane/diff/project validation.

Owned second-account permission candidate reference: `references/owned-second-account-permission-candidate.md` records the proof discipline for SaaS cases where a second operator-owned account appears as an active teammate/admin or has broad workspace permissions. Treat this as a strong candidate only after proving provenance or a low-privilege/removed-account negative control; do not overclaim expected membership as privilege escalation.

Second-account provenance chain-of-custody reference: `references/second-account-provenance-chain-of-custody.md` records the required A/B/C signup/invite/company-name/admin-approval provenance log and the clean Account C repro pattern. Use it whenever tenant/workspace/team permission testing depends on whether a secondary account self-joined, accepted an invite, or was manually approved.
...
10. If the selected candidate requires second account, OTP, CAPTCHA, billing, callback, token creation, invite email, or third-party communication, stop and request operator action/approval.
   - For second-account SaaS permission candidates, create a redacted provenance log before proceeding: account label, browser profile, signup source, same/different company label relation, invite state, admin approval/addition state, group/role assignment source, first post-auth screen, and evidence pointers. Unknown provenance is an evidence gap, not something to reconstruct later from operator memory.
   - If provenance is already missing or contaminated, stage a fresh Account C clean repro in a dedicated browser profile, stop at the operator secret gate, and resume after the operator completes signup/auth.
11. Execute only the bounded manual proof or stop with a preserved candidate list.
12. Update handoff artifacts and accepted changes.
13. Run validation appropriate to changed files: JSON/lane-state validation, evidence redaction check, diff check, project review if required.
...
16. Losing second-account provenance. For SaaS tenant/workspace/role findings, the join path is part of the proof, not background context. If you record only the final permission state and later ask the user whether the account self-registered, used the same company name, accepted an invite, or was approved by an admin, the evidence chain is already weakened. Log provenance as it happens; if it is missing, use a clean Account C repro instead of relying on memory.
